...
def base_population_evaluation(population, parsimony_coefficient, experiment, **kwargs):
    if experiment.casefold() == 'green':
        ...
        for controller in population:
            ...
            size = len(controller.genes.root.get_all_nodes())
            fitness = score - parsimony_coefficient * size
            base_fitness = score 
            
            if fitness is None or base_fitness is None:
                raise ValueError("Error in base_population_evaluation: fitness or base_fitness is None")

            controller.base_fitness = base_fitness
