...
class TreeNode:
    ...
    def validate_and_prune(self):
        ...
        if self.type in nonterminals:
            if not self.left:
                # Replace missing left child with a random terminal
                terminal = random.choice(terminals)
                self.left = TreeNode(terminal)
                if terminal == 'C':
                    self.left.value = random.uniform(-8, 8)
            else:
                self.left.validate_and_prune()
            
            if not self.right:
                # Replace missing right child with a random terminal
                terminal = random.choice(terminals)
                self.right = TreeNode(terminal)
                if terminal == 'C':
                    self.right.value = random.uniform(-8, 8)
            else:
                self.right.validate_and_prune()
        else:
            # If it's a terminal, it should not have any children
            self.left = None
            self.right = None
